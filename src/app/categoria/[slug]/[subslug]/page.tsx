...
// Utility to format context time
...
async function getSubcategoryNews(categorySlug: string, subcategorySlug: string) {
    const query = `*[_type == "post" && categorySlug == $categorySlug && subcategorySlug == $subcategorySlug && status == "published"] | order(publishedAt desc) {
        title,
        subtitle,
        lead,
    ...
    return await client.fetch(query, { categorySlug, subcategorySlug });
}
...
export default async function SubcategoryPage({ params }: { params: Promise<{ slug: string, subslug: string }> }) {
    ...
    // 2. Fetch real subcategory from Supabase
    ...
    // Also fetch sibling subcategories for the header
    const { data: siblingSubcategories } = await supabase
        .from("subcategories")
        .select("*")
        .eq("category_id", category.id)
        .order("name");

    const mappedSubcategories = (siblingSubcategories || []).map(sub => ({
        name: sub.name,
        href: `/categoria/${slug}/${sub.slug}`
    }));

    // 3. Fetch real news from Sanity for this subcategory
    const newsFromSanity = await getSubcategoryNews(slug, subslug);
                        ...
                        {mappedArticles.length === 0 && (
                            <div className="py-24 px-6 bg-white border border-dashed border-gray-200 rounded-sm text-center flex flex-col items-center justify-center">
                                ...
                                <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
                                    Aún no hay noticias publicadas en esta sección. Nuestro equipo editorial está trabajando para traerte la mejor información sobre {subcategory.name.toLowerCase()}.
                                </p>
                            </div>
                        )}
